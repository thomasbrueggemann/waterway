...
def pos_hash(position):
	raw = str(position["shipid"]) + "_" +  str(position["time"]) + "_" + json.dumps(position["coord"]["coordinates"]) + "_" + str(position["course"]) + "_" + str(position["speed"])
...
def get_positions(ship, page):
	...
	print "**** PAGE " + str(page) + " ****"
	url = "http://www.marinetraffic.com/en/ais/index/positions/all/shipid:" + str(ship["shipid"]) + "/mmsi:" + str(ship["mmsi"]) + "/shipname:" + ship["shipname"].replace(" ", "%20") + "/per_page:50/page:" + str(page)
